class ActiveRecord::Base
	...
	def erp_add_new_record_to_obj new_option={user: '', time: '', con: ''}, record_type = 'logger_info'
		obj = record_type == 'logger_info'? logger_info : remark
		old_info = obj.blank? ? [] : obj.split(',')
		new_info = new_option[:user,]+'/'+new_option[:time]+'/'+new_option[:con]
		old_info << new_info
	...
	def erp_get_obj_record_arr record_type = 'logger_info'
		...
		info_arr.map! do |r|
			r_arr = r.split('/')
			{user: r_arr[0], time: r_arr[1].to_time.strftime('%Y-%m-%d %H:%M:%S'), con: r_arr[2]}
		end
		info_arr
	end
end