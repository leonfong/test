...
=end
    ...
    def sell_baojia
        ...
        if not current_user.s_name.blank?
            ...
            @quate = ProcurementBom.find_by_sql("SELECT *,SUM(procurement_boms.t_p) AS sum_t_p FROM `procurement_boms` WHERE #{where_p + where_date + where_5star}  GROUP BY procurement_boms.p_name_mom ORDER BY created_at").paginate(:page => params[:page], :per_page => 10)
        else
            if params[:start_date] != "" and  params[:start_date] != nil
            ...
            if where_date != ""
                @quate = ProcurementBom.find_by_sql("SELECT *,SUM(procurement_boms.t_p) AS sum_t_p FROM `procurement_boms` WHERE #{where_date + where_5star}  GROUP BY procurement_boms.p_name_mom ORDER BY created_at").paginate(:page => params[:page], :per_page => 10)
            else
                if params[:complete]
                    @quate = ProcurementBom.find_by_sql("SELECT *,SUM(procurement_boms.t_p) AS sum_t_p FROM `procurement_boms` WHERE procurement_boms.star = 5   GROUP BY procurement_boms.p_name_mom ORDER BY created_at").paginate(:page => params[:page], :per_page => 10)
                else
                    @quate = ProcurementBom.find_by_sql("SELECT *,SUM(procurement_boms.t_p) AS sum_t_p FROM `procurement_boms`   GROUP BY procurement_boms.p_name_mom ORDER BY created_at").paginate(:page => params[:page], :per_page => 10)
                end
            end
        end
