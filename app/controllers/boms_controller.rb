...
=end                 
    ...
    def bomlist
        ...
        if can? :work_top, :all
            @boms = Bom.find_by_sql("SELECT * FROM `boms` ORDER BY `id` DESC ").paginate(:page => params[:page], :per_page => 10)
        end
    end
    ...
    def orderlist
        ...
        if can? :work_top, :all
            @order = Bom.find_by_sql("SELECT * FROM `orders` ORDER BY `id` DESC ").paginate(:page => params[:page], :per_page => 10)
        end
    end
