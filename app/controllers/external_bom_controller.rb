...
class ExternalBomController < ApplicationController
...
before_filter :authenticate_user!
    ...
    def p_upbom        
        if params[:bom_id] and params[:bak_bom].blank?
            old_bom = EItem.where(e_bom_id: params[:bom_id]) 
            old_bom.each do |old_item|
                old_item.destroy
            end
            @bom = EBom.find(params[:bom_id]) 
		...
		Rails.logger.info("------------------------------------------------------------des")
                ...
                Rails.logger.info("------------------------------------------------------------des")
                #find_mpn = EItem.where(e_bom_id: params[:bom_id],mpn: mpna)
                #if find_mpn.blank?
                    bom_item = @bom.e_items.build() #创建bom_items对象
            ...
            #return false
            @bom_item = EItem.where(e_bom_id: params[:bom_id])
        ...
        if not params[:bak_bom].blank? 
            ...
            #@sheet = @xls_file.sheet(0)
            #render "select_column.html.erb"
            redirect_to e_select_column_path(bom: @bom)
            return false
        end

        @bom = EBom.new(bom_params)#使用页面传进来的文件名字作为参数创建一个bom对象
        @bom.user_id = current_user.id
        @file = @bom.excel_file_identifier
        @bom.no = "MB" + Time.new.strftime('%Y').to_s[-1] + Time.new.strftime('%m%d').to_s + "B" + Bom.find_by_sql('SELECT Count(e_boms.id)+1 AS all_no FROM e_boms WHERE to_days(e_boms.created_at) = to_days(NOW())').first.all_no.to_s + "B"
        #如果上传成功
	if @bom.save
            #if @bom.excel_file_identifier.split('.')[-1] == 'xls'
	        #@xls_file = Roo::Excel.new(@bom.excel_file.current_path)
    ...
    #0.1如果有mpn
        ...
        #0.1.2如果自有物料不能匹配 
                                ...
                                if match_product_old.blank?
                                    ...
                                    match_dn = EDn.find(match_product_old.dn_id)
                                    add_dns = EDn.new
                                    add_dns.item_id = item.id
...
=end
            ...
            if not @bom.qty.blank?
                ...
                render inline: "window.location='/e_viewbom?bom_id=#{params[:bom_id]}';" 
            end           
        end      
    end
    ...
    def p_viewbom 
        ...
        @all_dn += "&quot;]"
        @boms = EBom.find(params[:bom_id])
        @user_do = "7"
        @bom_item = EItem.where(e_bom_id: params[:bom_id])
        
        if  params[:ajax]
            @bomitem = EItem.find_by_sql("SELECT id,mpn,part_code,quantity,price,(price*quantity) AS total,mf,dn FROM bom_items WHERE bom_items.id = '#{params[:ajax]}'").first
    ...
    def p_bomlist
        @boms = EBom.find_by_sql("SELECT * FROM `e_boms` WHERE `name` IS NULL ORDER BY `check` DESC,`created_at` DESC ").paginate(:page => params[:page], :per_page => 10)
    end
            ...
            if current_user.email == "web@mokotechnology.com"
                ...
                unless @match_products.nil?
                    @match_products[0..19].each do |item|
                        ...
                        @bom_html = @bom_html + "<a rel='nofollow' data-method='get' data-remote='true' href='/e_update?id="+ params[:id].to_s + "&product_id=" + item.id.to_s + "&bomsuse=bomsuse'><div>#{item.name.to_s}</div></a>"
                        @bom_html = @bom_html + "</td>"
                        @bom_html = @bom_html + "<td>"
                        @bom_html = @bom_html + "<a rel='nofollow' data-method='get' data-remote='true' href='/e_update?id="+ params[:id].to_s + "&product_id=" + item.id.to_s + "&bomsuse=bomsuse'><div>#{item.description.to_s}</div></a>"
                        @bom_html = @bom_html + "</td>"
                        @bom_html = @bom_html + "<td>"
                        @bom_html = @bom_html + "<a rel='nofollow' data-method='get' data-remote='true' href='/e_update?id="+ params[:id].to_s + "&product_id=" + item.id.to_s + "&bomsuse=bomsuse'><div>#{ActionController::Base.helpers.number_with_precision(item.min_price, precision: 4).to_s}</div></a>"
                        @bom_html = @bom_html + "</td>"
                        @bom_html = @bom_html + "<td>"
                        @bom_html = @bom_html + "<a rel='nofollow' data-method='get' data-remote='true' href='/e_update?id="+ params[:id].to_s + "&product_id=" + item.id.to_s + "&bomsuse=bomsuse'><div>OK</div></a>"
                        @bom_html = @bom_html + "</td>"
                        @bom_html = @bom_html + "</tr>"
                    end           
                end
        ...
        unless @package2 and @ptype
            ...
            unless @counted.nil?
                @counted.each do |key, value|
                    ...
                    Rails.logger.info("--------------------------")
                    @bom_lab = @bom_lab + '<a data-remote="true" href="/e_search_m' + "?id=" + @bom_item.id.to_s + "&amp;p=" + params[:p].to_s + "&amp;part_name=" + key + "&amp;q=" + params[:q].to_s + "&amp;bomsuse=bomsuse" + '">' + key.to_s + "</a>"
            ...
            unless @counted1.nil?
                @counted1.each do |key, value| 
                    ...
                    @bom_lab = @bom_lab + '<a data-remote="true" href="/e_search_m' + "?id=" + @bom_item.id.to_s + "&amp;p=" + params[:p].to_s + "&amp;package2=" + key.to_s + "&amp;q=" + params[:q].to_s + "&amp;bomsuse=bomsuse" + '">' + key.to_s + "</a>"
                    @bom_lab = @bom_lab + '<span class="badge">' + value.to_s + '</span>'
                end
            end
            @bom_lab = @bom_lab + "</td></tr>"
            ...
            unless @counted.nil? 
                @counted.each do |key, value| 
                    ...
                    Rails.logger.info("--------------------------")
                    @bom_lab = @bom_lab + '<a data-remote="true" href="/e_search_m' + "?id=" + @bom_item.id.to_s + "&amp;p=" + params[:p].to_s + "&amp;part_name=" + key.to_s + "&amp;q=" + params[:q].to_s + "&amp;bomsuse=bomsuse" + '">' + key + "</a>" 
                    @bom_lab = @bom_lab + '<span class="badge">' + value.to_s + '</span>'
                end
            end
            @bom_lab = @bom_lab + "</td></tr>"
            ...
            unless @counted1.nil?
                @counted1.each do |key, value|
                    #<%= link_to "#{key}",  edit_bom_item_path(@bom_item, :package2 =>key, :part_name =>@ptype, :q =>params[:q], :p =>params[:p]) %>
                     @bom_lab = @bom_lab + '<a data-remote="true" href="/e_search_m' + "?id=" + @bom_item.id.to_s + "&amp;p=" + params[:p].to_s + "&amp;package2=" + key.to_s + "&amp;q=" + params[:q].to_s + "&amp;part_name=" + @ptype + "&amp;bomsuse=bomsuse" + '">' + key.to_s + "</a>"
                    @bom_lab = @bom_lab + '<span class="badge">' + value.to_s + '</span>'
                end
            end
            @bom_lab = @bom_lab + "</td></tr>"
    ...
    def p_update
        if not params[:product_id].blank?
            del_dn = EDn.find_by_sql("SELECT * FROM e_dns WHERE e_dns.item_id = '#{params[:id]}' AND e_dns.tag IS NULL")
            #del_dn = EDn.find_by(item_id: params[:id], tag: nil)
            if not del_dn.blank?
                #del_dn.delete 
                #del_dn.save
...
=end
            @view_dns += '<tbody >'
            EDn.where(item_id: params[:id]).each do |dn|
                ...
                @view_dns += '<td width="50"><small><a rel="nofollow" data-method="get" data-remote="true" href="/e_updateii?id='+ params[:id].to_s + '&product_name=' + dn.part_code.to_s + '&dn_id=' + dn.id.to_s + '&bomsuse=bomsuse" ><div>' + dn.date.localtime.strftime('%y-%m').to_s + '</div></a></small></td>'
                
                @view_dns += '<td width="100" title="'+dn.dn_long.to_s+'"><small><a rel="nofollow" data-method="get" data-remote="true" href="/e_updateii?id='+ params[:id].to_s + '&product_name=' + dn.part_code.to_s + '&dn_id=' + dn.id.to_s +  '&bomsuse=bomsuse" ><div>' + dn.dn.to_s + '</div></a></small></td>'
                @view_dns += '<td width="50"><small><a rel="nofollow" data-method="get" data-remote="true" href="/e_updateii?id='+ params[:id].to_s + '&product_name=' + dn.part_code.to_s + '&dn_id=' + dn.id.to_s +  '&bomsuse=bomsuse" ><div>'+dn.qty.to_s+'</div></a></small></td>'
                @view_dns += '<td width="80"><small><a rel="nofollow" data-method="get" data-remote="true" href="/e_updateii?id='+ params[:id].to_s + '&product_name=' + dn.part_code.to_s + '&dn_id=' + dn.id.to_s +  '&bomsuse=bomsuse" ><div>￥'+dn.cost.to_s+'</div></a></small></td>'
                if not dn.info.blank?                
                    @view_dns += '<td width="80"><small><a href="'+dn.info.to_s+'">下载</a></small></td>'
                else
                    @view_dns += '<td width="80"><small></small></td>'
                end 
                @view_dns += '<td><small><a rel="nofollow" data-method="get" data-remote="true" href="/e_updateii?id='+ params[:id].to_s + '&product_name=' + dn.part_code.to_s + '&dn_id=' + dn.id.to_s +  '&bomsuse=bomsuse" ><div>' + dn.remark.to_s + '</div></a></small></td>'                
                @view_dns += '<td width="30"><small><a class="glyphicon glyphicon-trash" data-method="get" data-remote="true" href="/e_del_dn?id='+dn.id.to_s+'&item_id='+params[:id].to_s+'"></a></small></td>'
                @view_dns += '</tr>'
            ...
            Rails.logger.info("----------------------")
            #@view_dns = "wwwww"
            @bom_item = EItem.find(params[:id]) #取回e_items表bomitem记录，在解析bom是存入，可能没有匹配到product
            @bom = EBom.find(@bom_item.e_bom_id)
            if @bom_item.update_attribute("product_id", params[:product_id])
	            ...
	            @bom_item.save!
                    ...
                    if params[:bomsuse].blank?
                        ...
                        @match_str_nn = "#{@bom.e_items.count('product_id')+@bom.e_items.count('mpn_id')} / #{@bom.e_items.count}"
...
FROM
	e_items
LEFT JOIN products ON e_items.product_id = products.id
LEFT JOIN mpn_items ON e_items.mpn_id = mpn_items.id
WHERE
	e_items.e_bom_id = "+@bom_item.e_bom_id.to_s)             
                        @total_price_nn = 0.00               
    ...
    def p_updateii
        dell_dns_color = EDn.where(item_id: params[:id])
        c_color = nil
        dell_dns_color = EDn.where(item_id: params[:id]).update_all "color = '#{c_color}'"
        if not params[:product_name].blank?
            @add_dns = EDn.find(params[:dn_id])
            @add_dns.color = "b"
            @add_dns.save
            @bom_item = EItem.find(params[:id]) #取回e_items表bomitem记录，在解析bom是存入，可能没有匹配到product
            @bom = EBom.find(@bom_item.e_bom_id)
            if @bom_item.update_attribute("product_id", Product.find_by(name: params[:product_name]).id)
	            ...
	            @bom_item.save!
                    ...
                    if params[:bomsuse].blank?
                        ...
                        @match_str_nn = "#{@bom.e_items.count('product_id')+@bom.e_items.count('mpn_id')} / #{@bom.e_items.count}"
                        @matched_items_nn = EItem.where(e_bom_id: @bom.id)            
                        @total_price_nn = 0.00               
	                unless @matched_items_nn.empty?
                            @bom_api_all = []
		            @matched_items_nn.each do |item|
    ...
    def p_edit
        @bom_item = EItem.find(params[:item_id])
        if not @bom_item.blank?
            if not params[:info].blank?
                p_dn = EDn.new(bn_params)
            else
                p_dn = EDn.new()
            end
            #Rails.logger.info("--------------------------")
        ...
        if not @bom_item.blank?
            if @bom_item.user_do != params[:user_do]
                ...
                if not open_id.blank? or open_id != ""
                    ...
                    url += '&tips_url=www.fastbom.com/e_viewbom?bom_id='+@bom_item.e_bom_id.to_s 
    ...
    def up_check
        #check_user_do = EItem.where(e_bom_id: params[:bom_id],user_do: params[:user_do],user_do_change: "c")
        check_user_do = EItem.where("`e_bom_id` = #{params[:bom_id]} AND `user_do` = #{params[:user_do]} AND (`user_do_change` = 'c' OR `check` IS NULL OR `cost` IS NULL)")
        if not check_user_do.blank?
            redirect_to :back 
            return false
        else
            check_do = EItem.where(e_bom_id: params[:bom_id],user_do: params[:user_do]).update_all(check: "do")
            if can? :work_g_all, :all
                check_all = EItem.where(e_bom_id: params[:bom_id], check: nil)
                if check_all.blank?
                    EBom.find(params[:bom_id]).update(check: "do")
                end
            end
            redirect_to :e_bomlist 
            return false
        end
    end
    ...
    def p_excel
		...
		@bom.e_items.each_with_index do |item,index|
		    rowNum = index+1
		    ...
		    #row.push(item.quantity)
                    ...
                    if item.dn_id.blank?
                        ...
                        if EDn.find(item.dn_id).remark.blank?
                            row.push("")
                        else
                            row.push(EDn.find(item.dn_id).remark)
                        end
                    end
                    Rails.logger.info("qwqwqwqwqwqwqwqwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww")
                    ...
                    if not item.dn_id.blank?
                        ...
                        Rails.logger.info(EDn.find(item.dn_id).info_url.inspect)
                        Rails.logger.info("111111111111111")
                        if not EDn.find(item.dn_id).info_url.blank?
                            #row.push(request.protocol + request.host_with_port + EDn.find(item.dn_id).info_url)
                            row.push(Spreadsheet::Link.new request.protocol + request.host_with_port + EDn.find(item.dn_id).info_url, '技术资料')
                        else
                            row.push("")
                        end
                    else
    ...
    def p_edit_mpn
        item = EItem.find(params[:itemp_id])
        @p_item = item
    ...
    def copy_data
        source_data = EItem.find(params[:item_id])
        if not source_data.blank?
            update_data = EItem.where("e_bom_id = #{source_data.e_bom_id} AND description = '#{source_data.description.strip}' AND color != 'b'")
    ...
    private
        ...
        def search_bom_use (query_str)
            if not query_str.blank?
                result = EItem.find_by_sql("SELECT * FROM e_items WHERE e_items.description LIKE '%" + query_str.to_s.strip + "%' AND e_items.dn_id IS NOT NULL ORDER BY e_items.created_at DESC").first
            end
        end
