    ...
    def p_bomlist   
        ...
        if params[:order_list]
            ...
            if params[:key_order]
                ...
                @boms = ProcurementBom.find_by_sql("SELECT * FROM `procurement_boms` WHERE `p_name` LIKE '%#{params[:key_order].strip}%' AND `name` IS NULL AND `order_do` IS NULL ORDER BY `check` DESC #{order_ctl} ").paginate(:page => params[:page], :per_page => 15)
            else
                if params[:complete]
