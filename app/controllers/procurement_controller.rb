...
=end
    ...
    def other_baojia
        ...
        if can? :work_g, :all
            ...
            @bom_item = PItem.joins("JOIN procurement_boms ON procurement_boms.id = p_items.procurement_bom_id").where("p_items.user_do = '9999' AND quantity <> 0 AND procurement_boms.bom_team_ck = 'do' #{part_ctl} #{key}").order(add_orderby).paginate(:page => params[:page], :per_page => 15)
            if @bom_item.blank?
                @bom_item = PItem.joins("JOIN procurement_boms ON procurement_boms.id = p_items.procurement_bom_id").where("p_items.user_do = '9999' AND quantity <> 0 AND procurement_boms.bom_team_ck = 'do' #{part_ctl} #{key_des}").order(add_orderby).paginate(:page => params[:page], :per_page => 15)
            end
...
=end
    ...
    def part_list
        ...
        if can? :work_g_all, :all and can? :work_a, :all
            ...
            @user_do = "7"
            #@bom_item = PItem.where(user_do: "7")
            @bom_item = PItem.joins("JOIN procurement_boms ON procurement_boms.id = p_items.procurement_bom_id").where("(p_items.user_do = '7' OR (p_items.user_do = '999' AND p_items.supplier_tag = 'done')) AND quantity <> 0 AND procurement_boms.bom_team_ck = 'do' #{part_ctl} #{where_data}").order(add_orderby).paginate(:page => params[:page], :per_page => 15)
=begin
            if @bom_item.blank?
...
=end
            ...
            @user_do = "a1"
            @bom_item = PItem.joins("JOIN procurement_boms ON procurement_boms.id = p_items.procurement_bom_id").where("(p_items.user_do = 'a1' OR (p_items.user_do = '999' AND p_items.supplier_tag = 'done')) AND quantity <> 0 AND procurement_boms.bom_team_ck = 'do' #{part_ctl} #{where_data}").order(add_orderby).paginate(:page => params[:page], :per_page => 15)
...
=end
    ...
    def supplier_offer
        if can? :work_suppliers, :all
            ...
            if params[:complete]
                @part = PItem.where(user_do: '999',supplier_tag: 'done').order("updated_at DESC").paginate(:page => params[:page], :per_page => 10)             
